...
export default function NavBar({dark, handleDark}: NavBarProps) {
    ...
    const btnsToDisplay = homeBtns.map(({id, link}) => (
        <li 
            key={id}
            onClick={() => {}} 
            className="px-5 cursor-pointer capitalize hover:scale-110 duration-200"> {link} 
        </li>
    ))
    ...
    return (
        <div className={`flex justify-between items-center min-w-full h-20 fixed px-4 text-2xl
                ${dark ? "bg-white text-blue-950" : "bg-blue-900 text-white"}
                `
            }
        >
            <h1 className="ml-2 cursor-pointer hover:scale-110 duration-200 ">sean_balayan.io</h1>
            <div className="flex">
                <ul className="hidden items-center text-lg md:flex">
                    {homeBtns.map(({id, link}) => (
                        <li 
                            key={id}
                            onClick={typeof link !== "string" ? handleDark : handleClick} 
                            className="px-5 cursor-pointer capitalize hover:scale-110 duration-200"> {link} 
                        </li>
                    ))}
                </ul>
            ...
            {btnsOn &&
                <ul className={`
                        flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen 
